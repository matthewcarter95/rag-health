...
def update_session_google_connected(session_id: str, connected: bool = True) -> bool:
    """
    Update session google_connected flag after Auth0 Token Vault connection completes.

    Args:
        session_id: The session ID
        connected: True when Google is connected, False when disconnected
    ...
    try:
        sessions_table.update_item(
            Key={"session_id": session_id},
            UpdateExpression="SET google_connected = :gc",
            ExpressionAttributeValues={":gc": connected},
        )
        print(f"[Session] Updated google_connected={connected} for session: {session_id[:8]}...")
        return True
    except ClientError as e:
        print(f"[Session] Failed to update google_connected: {e}")
        return False
